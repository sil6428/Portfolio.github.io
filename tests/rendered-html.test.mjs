...
test("includes the device-local soundtrack and hidden terminal", async () => {
  ...
  assert.match(room, /daggerHeight = 1\.42/);
  ...
  assert.match(room, /bottom-shelf-printed-sword/);
  assert.match(room, /shelf-sword-blade/);
  assert.match(room, /shelf-sword-crossguard/);
  assert.match(room, /shelf-sword-pommel/);
