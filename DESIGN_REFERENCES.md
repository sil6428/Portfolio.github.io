...
| [Rachel Wei live portfolio](https://rachelqrwei.ca/use) | A room-based portfolio with content displayed over a persistent scene | The room, model set, copy, navigation, and interface are original. |
| [Rachel Wei source repository](https://github.com/rachelqrwei/personalwebsite) | Named hitboxes paired with visible props, grouped hover feedback, scene loading, an out-of-bounds reflection, and panels over one room | The repository has no included licence file. No code or assets were copied. This portfolio uses its own material-based hover system, responsive reflector, camera controls, and navigation. |
