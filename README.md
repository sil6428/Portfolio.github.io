...
- One locally hosted 1K glTF model from Poly Haven for the shelf camera. Its geometry is re-shaded with smooth physically based materials, restrained clearcoat, and shared environment lighting so the external camera and procedural objects fit one dark-studio visual language. The model is released under CC0 and documented in [THIRD_PARTY_ASSETS.md](THIRD_PARTY_ASSETS.md)
- A simplified floor plan without the armchair, side table, plant, or desk lamp, plus a larger cat rug and a workstation seated directly on the desk surface without decorative LED rails
