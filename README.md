...
- Two locally hosted 1K glTF models from Poly Haven for the shelf camera and floor plant. Their geometry is re-shaded with smooth physically based materials, restrained clearcoat, and shared environment lighting so external and procedural objects fit one dark-studio visual language. The models are released under CC0 and documented in [THIRD_PARTY_ASSETS.md](THIRD_PARTY_ASSETS.md)
- A simplified floor plan without the armchair, side table, or desk lamp, plus a larger cat rug and a workstation seated directly on the desk surface without decorative LED rails
